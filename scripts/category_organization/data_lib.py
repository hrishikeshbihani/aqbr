...
def get_table_list(product):
    cookies = {
        "t": "idfy|idfy/d27ef9f1-e537-473d-b9fa-ad9825bc29a4|2ba50040-1e1c-4b48-865e-e59ef66c7f00"}
    response=requests.get(f"https://explorer.shared.idfy.com/productTables?product={product}",cookies=cookies)
...
def make_explorer_api_call(url):
    ...
    if response.status_code == 200:
        ...
        raise Exception("None 200 Response from API. Status Code: {status_code}".format(status_code = response.status_code))

def get_table_schema_from_explorer(table_names):
    query_params = ",".join(table_names)
    url = "{url_path}/tabledata?tables={table_names_query_params}".format(table_names_query_params = query_params, url_path = os.getenv("EXPLORER_BASE_URL"))
...
def transform_json_schema_to_xml_schema(table_json):
    ...
    for table in table_json:
        ...
        table_tag = SubElement(schema, "table", name=table_name, description=table_description)
        ...
        for table_column in table_columns:
            ...
            column_tag = SubElement(table_tag, "column", is_nullable=is_nullable, data_type = data_type, description=description)
